...
void loop()
{
    ...
    {
        case ZERO_POSITION:
            Serial.println("【步骤1】将当前位置角度清零");
            smd_angle_to_zero(1);
            handle_ack(50);
            delay(500);
            
            Serial.println("位置清零完成，准备正向移动...");
            motionState = MOVE_TO_TARGET;
            break;
            
        case MOVE_TO_TARGET:
            Serial.println("【步骤2】正向移动到位置: 266252");
            smd_ol_pos_mode(1, 0, 200, 50, TargetPos);
            handle_ack(50);
            
            lastMoveState = MOVE_TO_TARGET;
            stateTimer = millis();
                ...
                if(result == 0)
                {
                    ...
                    if(frame.function_code == FCT_READ_ARRIVED_STA && frame.data_len > 0)
                    {
                        if(frame.data[0] == 1)
                        {
                            ...
                            if(lastMoveState == MOVE_TO_TARGET)
                            {
                                Serial.println("到达目标位置，准备返回零点...");
                                motionState = MOVE_TO_ZERO;
                            }
                            else if(lastMoveState == MOVE_TO_ZERO)
                            {
                                Serial.println("返回零点完成，准备重新开始...");
                                delay(1000);
                                motionState = MOVE_TO_TARGET;
                            }
                        }
        ...
        case MOVE_TO_ZERO:
            Serial.println("【步骤3】返回到位置: 0");
            smd_ol_pos_mode(1, 1, 200, 50, 0);
            handle_ack(50);
            
            lastMoveState = MOVE_TO_ZERO;
            stateTimer = millis();
            motionState = WAIT_ARRIVAL;
            break;
    }
