...
void _ui_screen_change(lv_obj_t ** target, lv_scr_load_anim_t fademode, int spd, int delay, void (*target_init)(void)) 
{
    ...
    // 遍历所有已知的屏幕指针，如果它不是当前要去的 target，就把它置 NULL
    // 这样下次回来时，if(*target == NULL) 才会生效，重新调用 init
    if(act_scr == ui_Screen1) ui_Screen1 = NULL;
    if(act_scr == ui_Screen2) { ui_Screen2 = NULL; ui_Chart2 = NULL; }
    if(act_scr == ui_Screen3) ui_Screen3 = NULL;
    if(act_scr == ui_Screen4) ui_Screen4 = NULL;
    if(act_scr == ui_Screen5) ui_Screen5 = NULL;
}
